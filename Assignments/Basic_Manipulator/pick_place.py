...
class manipulator():
    ...
    def forward(self , joint_angles ):
        ...
        # p.setJointMotorControlArray(self.bot,self.revolute_joints,p.POSITION_CONTROL,targetPositions = joint_angles )
        # joints_position = []
        # for j in self.revolute_joints:
        #     joints_position.append( p.getLinkState(self.bot , j)[:2] )
        # print( p.getLinkState(self.bot , 7)[:2][0])
    ...
    def move(self , joint_angles = None):
        ...
        # i = 100
        # while i > 0 :
        #     p.setJointMotorControlArray(self.bot,self.revolute_joints,p.POSITION_CONTROL,targetPositions = list(joint_angles) )
        #     time.sleep(1/50.)
        #     p.stepSimulation()
        #     i -= 1
        # return
    ...
    def plot(self):
        ...
        # axis = Axes3D(fig)
        # axis.set_xlim3d(-1.5, 1.5)
        # axis.set_ylim3d(-1.5, 1.5)
        # axis.set_zlim3d(-1.5, 1.5)
        # axis.set_xlabel('$X$')
        # axis.set_ylabel('$Y$')
        # # axis.set_zlabel('$Z$')
        # axis.set_title("3D simulation")
        # for i in range(len(self.debug_list)):
        # p1, = axis.plot(self.debug_list[0],self.debug_list[1],self.debug_list[2],'ro')
        plt.title("x vs t")
        p1 = plt.plot( self.debug_list[0] , 'ro')
            # camera.snap()
        plt.show()
        plt.title("y vs t")
        p1 = plt.plot( self.debug_list[1] , 'ro')
            # camera.snap()
        plt.show()
        plt.title("z vs t")
        p1 = plt.plot( self.debug_list[2] , 'ro')
            # camera.snap()
        plt.show()
    ...
    def run_simulation(self , limit , ini_pos ,obj_pos , fin_pos):
        t1 = time.time()
...
if __name__ == '__main__' :
    ...
    arm.run_simulation( limit , ini_pos ,obj_pos , fin_pos)
